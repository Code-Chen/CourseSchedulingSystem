...
class CollegeInfo extends PureComponent {
    state = {tableData:[]};
    ...
    columns = [
        ...
        {title: '学院编号', dataIndex: 'collegeno',},
        {title: '学院名称', dataIndex: 'collegename',},
    ];

    getTableData = () => {

        axios.post('/queryCollegeInfo', ).then((res) => {
            if (res.data.code === '1') {
        ...
        return (
            <Modal visible={visible} width={800} onOk={() => onOk(this.selectData)} onCancel={onCancel}>
                ...
                >
                    ...
                    <Table columns={this.columns} dataSource={this.tableData} rowSelection={rowSelectProp} bordered/>
                </Form>
            </Modal>
        )
    }
...
export default Form.create()(CollegeInfo);